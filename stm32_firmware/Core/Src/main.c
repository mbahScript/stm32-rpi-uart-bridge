...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : STM32F767 ↔ RPi5 UART Transport Protocol v0.3.1 (USART3)
  ******************************************************************************
...
static char     last_reply_frame[160];
...
/**
 * @brief Send a framed reply that echoes HOST SEQ AND stores it.
 *        v0.3.1: Allows re-sending the last reply when host retries same SEQ.
 */
static void send_packet_reply_store(const char *type, const char *node, uint8_t seq, const char *data)
{
    char payload[128];
    ...
    *last_bar = '\0'; // now copy = "TYPE|NODE|SEQ|DATA"
    ...
    if (strcmp(type, "CMD") == 0) {
        if (host_seq == last_host_seq) {
            /* v0.3.1: host retried same SEQ -> re-send last stored reply */
            if (last_reply_len > 0) {
                HAL_UART_Transmit(&huart3, (uint8_t*)last_reply_frame, last_reply_len, HAL_MAX_DELAY);
            }
            return;
